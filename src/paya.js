 ...
 * @name        Paya Calendar
 * @version     2.0.0
 * @description Solar calander base
...
(function() {
    ...
    if (typeof terminal !== 'function') {
        console.error('terminal.js is missing');
...
class paya extends terminal{
    ...
    constructor(config = {}, consoleConfig = {}) {
        ...
        this.#consoleSettings   = consoleConfig;

        this.#element       = this.#settings.element;

        if (this.constructor === paya) {
            if($ID(this.#element) instanceof HTMLDivElement) {
                if (this.constructor.prototype === paya.prototype) {
                    this.#create();
                }
            }
            else {
    ...
    */
    #isDate() {
        ...
        if(!$TYPE(arguments[0], 'undefined')) if(!$TYPE(arguments[0], 'string')) { return false; }

        if (!/^\d{4}-\d{2}-\d{2}$/.test(arguments[0])) return false;

        const _date = new Date(arguments[0]);
        return _date instanceof Date && !isNaN(_date) && _date.toISOString().startsWith(arguments[0]);
    }
    ...
    */
    ...
    calDegree(x1, y1, x2, y2) {
        return this.#calDegree(x1, y1, x2, y2);
    }
    ...
    #create() {
        ...
        const caller    = stack[3]?.includes('paya') ? true : false;

        const termSettings = caller ? { ...this.#settings, ...termDefaults } : { ...termDefaults, ...this.#consoleSettings };
    ...
    */
    #update() {
        ...
        let observDateErr   = new Date('2023-08-27T00:00:00Z'),
        MoonobservDateErr   = new Date('2023-09-01T00:00:00Z'),
        nowDateErr          = new Date(Date.now()),
        ...
        let loop = 1;
        for (const planet in this.#heliocentricLongitude) {
            let wFormule            = diffDateErr * (360 / (Number(this.#planets[loop].orbitalPeriod) / 365.25));
            geoLongitude[planet]    = ((this.#heliocentricLongitude[planet] + (wFormule)) + 180) % 360;
        ...
        let vFormule        = (nowDateErr - MoonobservDateErr) / (1000 * 60 * 60) * (360 / (Number(this.#moon.orbitalPeriod)));
        ...
        $ID('data1').TEXT(`Zodi: ${this.#addZero(Number(zodiacDate.zodiacYear))}/${this.#addZero(Number(zodiacDate.zodiacMonth) + 1)}/${this.#addZero(Number(zodiacDate.zodiacDay))}`);
        ...
        $ID('data4').TEXT(`Greg: ${this.#addZero(new Date(this.#nowDate).getFullYear())}/${this.#addZero(new Date(this.#nowDate).getMonth() + 1)}/${this.#addZero(new Date(this.#nowDate).getDate())}`);
    ...
    */
    #ZodiacDate() {
        ...
        return {
            zodiacYear:     partSplit[0],
            zodiacMonth:    partSplit[1],
            zodiacDay:      partSplit[2]
        };
    }
    ...
    */
    ZodiacSign() {
        return this.#ZodiacSign(arguments[0]);
    }
    ...
    */
    #ZodiacSign() {
        ...
        for (let i = 0; i < this.#zodiac.length; i++) {
            const curr = this.#zodiac[i];
            const next = this.#zodiac[(i + 1) % this.#zodiac.length];

            const inCurr = (m === curr.month && d >= curr.day);
            const inNext = (m === next.month && d < next.day);

            if (inCurr) return curr.name;

            if (inNext) return curr.name;
        }

        return this.#zodiac[0].name;
    }
    ...
    #defaultCommand() {
        ...
        this.addCommand('get', function () {
            ...
            if(!$TYPE(arguments[0][1], 'undefined')) {
                ...
                if (arguments[0][1].toLowerCase() == 'date' && $TYPE(arguments[0][2], 'undefined')) {
                    ...
                    $TIMEOUT(this.log(`Paya: ${payaDate.payaColossalYear}.${payaDate.payaLargeYear}.${payaDate.payaYear}.${payaDate.payaMonth}.${payaDate.payaDay}<br>`), 100);
                    $TIMEOUT(this.log(`Maya: ${mayaDate.mayaBaktun}.${mayaDate.mayaKatun}.${mayaDate.mayaTun}.${mayaDate.mayaWinal}.${mayaDate.mayaKin}<br>`), 200);
                    $TIMEOUT(this.log(`Zodi: ${this.#addZero(Number(zodiacDate.zodiacYear))}/${this.#addZero(Number(zodiacDate.zodiacMonth) + 1)}/${this.#addZero(Number(zodiacDate.zodiacDay))}<br>`), 300);
                    $TIMEOUT(this.log(`Sola: ${this.#addZeroFA(solarDate.solarYear)}/${this.#addZeroFA(solarDate.solarMonth)}/${this.#addZeroFA(solarDate.solarDay)}<br>`), 400);
                    $TIMEOUT(this.log(`Luna: ${this.#addZeroAR(lunarDate.hijriYear)}/${this.#addZeroAR(lunarDate.hijriMonth)}/${this.#addZeroAR(lunarDate.hijriDay)}<br>`), 500);
                    $TIMEOUT(this.log(`Greg: ${this.#addZero(new Date(this.#nowDate).getFullYear())}/${this.#addZero(new Date(this.#nowDate).getMonth() + 1)}/${this.#addZero(new Date(this.#nowDate).getDate())}`), 600);      
                }
                if (arguments[0][1].toLowerCase() == 'date' && !$TYPE(arguments[0][2], 'undefined')) {
                    switch(arguments[0][2].toLowerCase()) {
                        case 'paya':
                            let payaDate    = this.#PayaDate(new Date(this.#nowDate));
                            this.log(`Paya: ${payaDate.payaColossalYear}.${payaDate.payaLargeYear}.${payaDate.payaYear}.${payaDate.payaMonth}.${payaDate.payaDay}<br>`);
                            break;
                        case 'maya':
                            let mayaDate    = this.#MayaDate(new Date(this.#nowDate));
                        ...
                        case 'zodi':
                            let zodiacDate  = this.#ZodiacDate(new Date(this.#nowDate));
                            this.log(`Zodi: ${this.#addZero(Number(zodiacDate.zodiacYear))}/${this.#addZero(Number(zodiacDate.zodiacMonth) + 1)}/${this.#addZero(Number(zodiacDate.zodiacDay))}<br>`);
                            break;
                        case 'sola':
                            let solarDate   = this.#SolarDate(new Date(this.#nowDate));
                        ...
                        case 'luna':
                            ...
                            break;
                        case 'greg':
                            this.log(`Greg: ${this.#addZero(new Date(this.#nowDate).getFullYear())}/${this.#addZero(new Date(this.#nowDate).getMonth() + 1)}/${this.#addZero(new Date(this.#nowDate).getDate())}<br>`);
                            break;
                        default:
                            this.log(`Greg: ${this.#addZero(new Date(this.#nowDate).getFullYear())}/${this.#addZero(new Date(this.#nowDate).getMonth() + 1)}/${this.#addZero(new Date(this.#nowDate).getDate())}<br>`);
                            break;
         ...
         , ' get &lt;date|time&gt; <br> get &lt;date&gt; &lt;paya|maya|zodi|sola|luna|greg&gt; <br>  get &lt;sign&gt; &lt;name&gt; &lt;adj&gt; <br>  get &lt;planet&gt; &lt;name&gt; &lt;zod&gt;');
        ...
        this.addCommand('set', function () {
            ...
            if(!$TYPE(arguments[0][1], 'undefined') && arguments[0][1].toLowerCase() == 'date') {
                ...
                else if (!$TYPE(arguments[0][2], 'undefined') && this.#isDate(arguments[0][2]) == true) {
                    this.update(arguments[0][2]);
                }
                else if (!$TYPE(arguments[0][2], 'undefined') && arguments[0][2].toLowerCase() == 'reset') {
                    this.update('reset');            
                }
            }                     
        }, 'update cosmos'
    ...
    returnMoonLongitude() {
        return this.#moonLongitude;
    }
    
    hideData() {
        $ID(this.#hexID.dataBodyID).PARENT().HIDE();
    }
