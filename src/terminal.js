...
class terminal {
     ...
     *  @param {array} Resize Axies
     *  @param {number} Max Lines
     *  @param {number} Max History
     *  @param {number} Wait Input
     */
    constructor(config = {}) {
        const defaults = {
            element: 'terminal',
        ...
        if (this.constructor === terminal) {
            if($ID(this.#element) instanceof HTMLDivElement) {
                if (this.constructor.prototype === terminal.prototype) {
                    this.#create();
                }
            }
            else {
     ...
     */
    #hexRGB() {
        if(!arguments[0]) { $ERROR('require args'); return; }

        if(!$TYPE(arguments[0], 'string')) { $ERROR('arg type mismatch'); return; }

        const validHex      = /^#?([a-f\d]{3}|[a-f\d]{6})$/i.test(arguments[0]);
        if (!validHex) { $ERROR('invalid hex color'); return; }

        const normalized    = arguments[0].replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i, (_, r, g, b) => `${r}${r}${g}${g}${b}${b}`);
    ...
    #split() {
        ...
        for (let i = 0; i < arguments[0].length; i++) {
            const ch = arguments[0][i];

            if (quoteChar && ch === '\\' && i + 1 < arguments[0].length) {
                current += arguments[0][++i];
                continue;
            }

            if (!quoteChar && (ch === '"' || ch === "'" || ch === '`')) {
                quoteChar = ch;
                continue;
            }
    ...
    */
    ...
    #create() {
        ...
        this.#typeModeChars = !this.#settings.dateOption ? '$' : '[00:00:00:00]$';

        terminal_style.innerHTML = `
            #${this.#hexID.input}::before{ content: '${this.#typeModeChars}'; color: #999999; }
            ...
            #${this.#element} a{color: #3b3b3b; text-decoration: none; cursor: pointer;}
            #${this.#element} a:hover, #${this.#element} a:focus {color: #6b6b6b;}`;
        ...
        const execute = () => {
            if(this.#settings.dateOption) {
                $ID(terminal_text.id).insertAdjacentHTML('beforeend', `[00:00:00:00]$${this.#sanitizeText($ID(terminal_input.id).TEXT())}<br>`);
            }
            else {
                $ID(terminal_text.id).insertAdjacentHTML('beforeend', `$${this.#sanitizeText($ID(terminal_input.id).TEXT())}<br>`);
            }
            $ID(terminal_text.id).scrollTo(0, $ID(terminal_text.id).scrollHeight);            
            this.#addCommandHistory(this.#sanitizeHTML($ID(terminal_input.id).TEXT()));            
            this.#execute(this.#split($ID(terminal_input.id).TEXT()));
            $ID(terminal_input.id).TEXT('');
        }

        const returnInput = () => {
            this.#privateData   = $ID(terminal_input.id).TEXT();
            this.#waitInput     = false;
            $ID(terminal_input.id).TEXT('');           
        }

        const returnPrivateInput = () => {
            this.#privateData       = this.#privateKeyStore;
            this.#privateKeyStore   = '0'.repeat(this.#privateKeyStore.length);
        ...
        for(let i = 0; i <= (elements.length - 1); i++){
            ...
            $ID(elements[i]).ontouchstart = (event) => {
                focus();
        ...
        const historyUp = () => {
            ...
            if (this.#historyIndex === -1) {
                this.#historyDraft = $ID(terminal_input.id).TEXT();
            }
            ...
            if (this.#historyIndex > 0) {
                this.#historyIndex -= 1;
                $ID(terminal_input.id).TEXT(this.#commandsHistory[this.#historyIndex]);
                focus();
            }
        }
        ...
        const historyDown = () => {
            ...
            if (this.#historyIndex >= this.#commandsHistory.length) {
                this.#historyIndex = -1;
                $ID(terminal_input.id).TEXT(this.#historyDraft);
                this.#historyDraft = '';
            } else {
                $ID(terminal_input.id).TEXT(this.#commandsHistory[this.#historyIndex]);
            }
        ...
        $ID(this.#hexID.input).onkeyup = (event) => {
            if (!this.#waitPrivateInput) return;

            event.preventDefault();

            if (event.inputType === 'deleteContentBackward') {
                this.#privateKeyStore = this.#privateKeyStore.slice(0, -1);
            } else if (event.key && event.keyCode >= 33 && event.keyCode <= 254) {
                this.#privateKeyStore += event.key;
            }
    ...
    */
    #validateIP() {
        ...
        const ipv4Regex = /^(([01]?\d{1,2}|2[0-4]\d|25[0-5])\.){3}([01]?\d{1,2}|2[0-4]\d|25[0-5])$/;
    
        const ipv6Regex = /^(([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,7}:|([0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|([0-9a-fA-F]{1,4}:){1,5}(:[0-9a-fA-F]{1,4}){1,2}|([0-9a-fA-F]{1,4}:){1,4}(:[0-9a-fA-F]{1,4}){1,3}|([0-9a-fA-F]{1,4}:){1,3}(:[0-9a-fA-F]{1,4}){1,4}|([0-9a-fA-F]{1,4}:){1,2}(:[0-9a-fA-F]{1,4}){1,5}|[0-9a-fA-F]{1,4}:((:[0-9a-fA-F]{1,4}){1,6})|:((:[0-9a-fA-F]{1,4}){1,7}|:)|fe80:(:[0-9a-fA-F]{0,4}){0,4}%[0-9a-zA-Z]{1,}|::(ffff(:0{1,4}){0,1}:){0,1}((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])|([0-9a-fA-F]{1,4}:){1,4}:((25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9])\.){3}(25[0-5]|(2[0-4]|1{0,1}[0-9]){0,1}[0-9]))$/;
        
        if(arguments[1] === 'v4' || arguments[1] === 4) {
            return ipv4Regex.test(arguments[0]);
        }
        
        if(arguments[1] === 'v6' || arguments[1] === 6) {
            return ipv6Regex.test(arguments[0]);
        }
        
        return ipv4Regex.test(arguments[0]) || ipv6Regex.test(arguments[0]);
    }
    ...
    */
    ...
    #sanitizeText() {
        ...
        return String(arguments[0]).replace(/[&<>"'`]/g, (char) => map[char]);
    }
     ...
     */
    #sanitizeHTML() {
        const ALLOWED_TAGS = ['a', 'b', 'i', 'u', 'br', 'span', 'strong', 'em', 'p'];
        ...
        const isSafeHref = (val) =>
            /^(https?:\/\/|mailto:)/i.test(val.trim());

        const sanitizeStyle = (rawStyle) => {
            return rawStyle
                .split(';')
                .map(decl => {
                    ...
                    if (!SAFE_CSS_PROPS.has(cleanProp)) return null;
                    return `${cleanProp}:${rest.join(':').trim()}`;
                })
                .filter(Boolean)
                .join(';');
        };
        ...
        const clean = (node) => {
            ...
            if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                (ALLOWED_ATTRS[tag] || []).forEach(attr => {
                    ...
                    if (attr === 'href' && !isSafeHref(val)) return;
                    if (attr === 'style') val = sanitizeStyle(val);
        ...
        tempElement.innerHTML   = arguments[0];

        const frag              = document.createDocumentFragment();
        tempElement.childNodes.forEach(child => frag.appendChild(clean(child)));
    ...
    #trimLog() {
        const container = document.getElementById(this.#hexID.text);
        while (container.children.length > this.#settings.maxLines) {
    ...
    #log() {
        ...
        if(arguments[0] != null) {
            const safeColor = /^#[0-9a-fA-F]{3,6}$|^[a-zA-Z]+$/.test(color) ? color : this.#settings.fgColor;

            $ID(this.#hexID.text).insertAdjacentHTML('beforeend', `<div style="color:${safeColor};" id="${logID}"></div>`);
            this.#lineID.set(logID);

            arguments[0] = this.#sanitizeHTML(arguments[0]);
            ...
            if(this.#settings.dateOption == true)
            {
                ...
                $ID(logID).insertAdjacentHTML('beforeend', `<span style="color: gray;">${dateString}</span>`);
            }
            if(this.#settings.typeMode == true || arguments[2] == true)
            {
                if(arguments[0].includes('<br>')) {
                    $ID(logID).insertAdjacentHTML('beforeend', arguments[0].toString());
                }
                else
                {
                    let typeCounter     = 0;
                    const typeText      = arguments[0].toString();
                    const typeElement   = $ID(logID);
                    const typeInterval  = setInterval(() => {
                        if (typeCounter >= typeText.length) {
                            clearInterval(typeInterval);
                        } else {
                            $ID(logID).insertAdjacentText('beforeend', typeText.charAt(typeCounter));
                            typeCounter++;
                        }     
                    }, 15);
                } 
            ...
            if(arguments[0].length > 0) { 
                $ID(this.#hexID.text).insertAdjacentHTML('beforeend', `<div style="font-size:4px;"><br></div>`);
            }

            this.#trimLog();

            if(this.#settings.typeMode == true) {
                setTimeout(() => {
    ...
    #addCommandHistory() {
        ...
        if(arguments[0] != null && arguments[0] != '') {
            if(this.#commandsHistory.length == 0){
                this.#commandsHistory.push(arguments[0].toString());
            }
            else {
                if(this.#commandsHistory[this.#commandsHistory.length - 1] != arguments[0].toString()){
                    if (this.#commandsHistory.length >= this.#settings.maxHistory) {
    ...
    #defaultCommand() { 
        ...
        this.#addCommand('history', function () {
            let _data = '';
            for(let i = 0; i <= (this.#commandsHistory.length - 1); i++){
        ...
        this.#addCommand('help', function () {
            let _data       = 'help instruction<br>';
            let _dateOption = !this.#settings.dateOption ? '' : '[00:00:00:00]';
            for(let [key, value] of this.#commandsHelp){
                _data += `${_dateOption}${key}: ${value}<br>`;
            }
            if($TYPE(_data, 'empty'))
    ...
    #extraCommand() {       
        this.#addCommand('date', function () {
            let date = new Date();
            this.#log(`${date.getFullYear()}/${this.#addZero(date.getMonth() + 1)}/${this.#addZero(date.getDate())}`);
        }, 'show date'
         , '');
        ...
        this.#addCommand('color', function () {
            if(arguments[0].length <= 1) { this.#log('require args', '#CC0000'); return; }
            ...
            if(this.#validateHex(arguments[0][1]))
            {
                ...
                this.#settings.bgColor              = `#${arguments[0][1]}`;
            }
            if(this.#validateHex(arguments[0][2]))
            {
                ...
                this.#settings.fgColor              = `#${arguments[0][2]}`;        
            }
        ...
        this.#addCommand('ping', function () {           
            ...
            if(this.#validateURL(arguments[0][1]) || this.#validateIP(arguments[0][1]) || arguments[0][1] == 'localhost')
            {
                ...
                const options = {
                    method: 'GET',
                    url: `https://api.allorigins.win/get?url=${encodeURIComponent(arguments[0][1])}`,
                    cache: false,
                    timeout: 5000,
        ...
        this.#addCommand('systeminfo', async function () {            
            ...
            const cookies           = () => { return document.cookie.split(';').map(c => decodeURIComponent(c.trim())); }
            const gl                = document.createElement('canvas').getContext('webgl');
            const fontList          = () => {
                const fontListValue     = Array.from(document.fonts).map(font => font.family);
                return fontListValue.length > 0 ? fontListValue : [];
            }
        
            let info = {            
                userAgent:          navigator.userAgent || 'unknown',
                cookieEnabled:      navigator.cookieEnabled || false,
                language:           navigator.languages || 'unknown',
                userLanguage:       navigator.language  || 'unknown',
                ...
                referrerUrl:        document.referrer || 'unknown',
                ...
                localStorage:       `${(JSON.stringify(localStorage).length * 16) / (8 * 1024)} KB` || 'unknown',
                sessionStorage:     `${(JSON.stringify(sessionStorage).length * 16) / (8 * 1024)} KB` || 'unknown',
                ...
                plugins:            Array.from(navigator.plugins).map(plugin => plugin.name) || 'unknown',
            ...
            if(navigator.connection) {
                ...
                info.rtt            =   navigator.connection.rtt || 'unknown',
                info.downlink       =   navigator.connection.downlink || 'unknown',
        ...
        this.#addCommand('file', async function () {
            ...
            if (arguments[1] === 'get') {
                ...
                if (!$TYPE(arguments[2], 'undefined')) if (!$TYPE(arguments[2], 'string')) { $ERROR('arg type mismatch'); return; } else {
                    try {
                        ...
                        this.#log('crypto operation failed');
                        return '';
                    }
                }
            }
        ...
        this.#addCommand('hashsum', async function () {
            ...
            const algorithm = arguments[0][1].toLowerCase() || 'md5';
         ...
         this.#addCommand('crypt', async function () {
            ...
            fileInput.onchange = async (e) => {
                ...
                try {
                    ...
                    const iv = mode === 'enc' ? crypto.getRandomValues(new Uint8Array(12)) : data.slice(0, 12);
                    const content = mode === 'enc' ? data : data.slice(12);
            ...
            fileInput.click();
        }, 'encrypt or decrypt files (AES-GCM)'
         , 'crypt &lt;enc|dec&gt; [&lt;key&gt;]');
    ...
    async #getInput() {
        ...
        return new Promise(resolve => {
            const timeout = setTimeout(() => {
                ...
                resolve('');
                this.#privateData = '';
            }, this.#settings.waitInput);
            const interval = setInterval(() => {
                if (this.#privateData !== '') {
                    clearInterval(interval);
                    resolve(this.#privateData);
                    this.#privateData   = '0'.repeat(this.#privateData.length);
                    this.#privateData = '';
                }
            }, 200);
        });
